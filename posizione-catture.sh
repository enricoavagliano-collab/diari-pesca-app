...
echo "=== Aggiungo la posizione GPS ad ogni cattura ==="
...
cat > lib/diario-entries.ts << 'DIARIOENTRIESEOF'
import { sql } from "./db";
...
echo "  lib/diario-entries.ts"
echo ""
echo "Novita': nella tab Catture, mentre componi una nuova cattura, c'e' ora"
echo "un pulsante \"Aggiungi la posizione della cattura\" - un tap rileva la"
echo "posizione GPS esatta del telefono in quel momento e la salva insieme"
echo "a specie/foto/note. Le catture con posizione mostrano un link"
echo "\"Vedi posizione\" che apre Google Maps sul punto esatto."
echo ""
echo "Ricorda: bash posizione-catture.sh, poi:"
echo "git add -A && git commit -m 'aggiunge posizione GPS a ogni cattura' && git push"
