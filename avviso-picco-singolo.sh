        ...
        {selected && !loadingData && tides && (
          <>
            <div className="bg-[#0F2D3D] text-[#F6F5F1] rounded-xl p-5 mb-2">
              ...
              {tides.length === 1 && (
                <p className="text-[11px] text-[#a9bcc2] mt-3 leading-relaxed">
                  Il modello ha rilevato un solo picco oggi in questa zona — capita nei punti costieri più
                  complessi (es. foci) dove il segnale di marea è meno netto. Non è un errore: il resto della
                  giornata è semplicemente più piatto.
                </p>
              )}
            </div>
